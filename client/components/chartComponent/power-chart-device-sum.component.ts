...
export class PowerChartDeviceSumComponent {
    ...
    set chartDevices(chartDevices) {
        var devicesSize = _.size(chartDevices);
        if (devicesSize && this.oldLength !== devicesSize) {
            var ac_connected = [];
            var app_running = [];
            ...
            var remaining_time = [];
            var voltage = [];
            this.oldLength = devicesSize;
            var chartDevicesList = _.values(chartDevices)
            _.forEach(chartDevicesList[0], function (powerProfile, key) {
                ...
                var tempVoltage = 0;
                chartDevicesList.forEach((chartDevice)=> {
                ...
                ac_connected.push(Math.round(tempACConnected / this.oldLength));
                app_running.push(Math.round(tempAppRunning / this.oldLength));
                computer_running.push(Math.round(tempComputerRunning / this.oldLength));
                power_rate.push(Math.round((tempPowerRate / this.oldLength) * 100) / 100);
                remaining_capacity.push(Math.round(tempRemainingCapacity / this.oldLength));
                remaining_time.push(Math.round(tempRemainingTime / this.oldLength));
                voltage.push(Math.round((tempVoltage / this.oldLength) * 100) / 100);
            });
            this.lineChartData = [
